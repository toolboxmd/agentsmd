...
| `to-spec` | ToolboxMD / AgentsMD | Matt Pocock, Matt pin, `skills/engineering/to-spec` | Active | [MIT](LICENSES/mattpocock-skills-MIT.txt) | Model-discoverable entry to the complete Specify workflow through approved parent and verified ticket publication, with a Parent Spec only opt-out. |
| `to-tickets` | ToolboxMD / AgentsMD | Matt Pocock, Matt pin, `skills/engineering/to-tickets` | Active | [MIT](LICENSES/mattpocock-skills-MIT.txt) | Model-discoverable composition step that publishes an approved native Issue graph, then reuses or requests implementation authority at the first unblocked Issue. |
