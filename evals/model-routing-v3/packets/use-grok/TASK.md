...
```bash
/usr/bin/python3 -B .benchmark/public/use_grok_three_host.py --workspace .
```

The public verifier is read-only benchmark input. Candidate-owned tests are
supplemental evidence only. Do not change the skill, CLI reference, license,
wiki, public verifier, or public test runner.
