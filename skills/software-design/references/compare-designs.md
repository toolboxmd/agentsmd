...
Choose the cheapest artifact that can distinguish candidates: a script, type
check, benchmark, state simulator, or UI prototype. Record the hypothesis,
predicted observation, conditions, and falsifier before running it. Route a
human-owned experience decision through `prototype`; an empirical check needs
no artificial human verdict.
