...
Issue #6 is implemented in an isolated branch from exact Issue #8 PR head
`a771a7e9b5bc8036f47ed320c7b183d0da17e4bc`. This is an explicitly
coordinated stack: Issue #6 can target Issue #8's branch and present only its
own diff while PR #10 remains open. PR #10 itself targets PR #9's branch.

The weak two-bullet candidate is replaced by the ordered closed loop, and the
overlapping Proof cadence is consolidated into final-seam guidance. The exact
pre-edit file is preserved at
`agentsmd-archive/AGENTS.md.2026-08-29T15-12-14+0200.bak`.
The behavior is backward compatible and is prepared as version 2.4.0. Issue #5
has not begun an observed implementation branch or PR and should follow the
final same-file sequence.

PR #9 merged at `874b1bb8fb5c9a4f31e2ca658f768a7f3dfcf4b9`, and v2.2.0
was released while this correction was in progress. PR #10 remains open at
`a771a7e9b5bc8036f47ed320c7b183d0da17e4bc` with version 2.3.0. Both
refreshed Version check runs are green, and GitHub reports CLEAN and MERGEABLE.
PR #10 still targets the already-merged PR #9 branch instead of `main`, so it
must not merge in its current base configuration. Its owner must retarget it to
`main`, verify that the seven-file content diff and green readiness remain,
then merge and release v2.3.0 under the Human Gate. The current merge-base of
`origin/main` and `a771a7e` is `bbcde02`, so the content diff should remain the
same after retargeting, but that external mutation still needs post-change
proof. Exact-base content proof can complete on this branch.

This sequencing is a real application of the method: do not accelerate into
concurrent same-file work or automate around the ownership constraint. Resolve
or explicitly order the active constraint first.
